# Localink - Deployment Guide

This comprehensive guide covers all deployment strategies, infrastructure setup, and operational procedures for the Localink application.
...
Localink supports three production-ready deployment strategies:
...
# Verify namespace
kubectl get namespace localink
```
...
kubectl create secret generic localink-secrets \
...
# Verify secrets
kubectl get secrets -n localink
```
...
# Verify config
kubectl get configmap localink-config -n localink
```
...
kubectl set image deployment/localink-frontend \
  frontend=registry.io/localink-frontend:v1.2.0 \
  -n localink

# Watch rollout status
kubectl rollout status deployment/localink-frontend -n localink
```
...
kubectl rollout undo deployment/localink-frontend -n localink
```
...
```bash
# Check pods
kubectl get pods -n localink

# Check deployments
kubectl get deployments -n localink

# Check services
kubectl get services -n localink

# Check resource usage
kubectl top pods -n localink
kubectl top nodes
```
...
```bash
# Frontend logs
kubectl logs -f deployment/localink-frontend -n localink

# Backend logs
kubectl logs -f deployment/localink-backend -n localink

# Stream logs from all pods
kubectl logs -f -l app=localink -n localink
```
...
# Kubectl rollback
kubectl rollout undo deployment/localink-frontend -n localink
kubectl rollout undo deployment/localink-backend -n localink
```
...
**Diagnosis**:
```bash
kubectl describe pod <pod-name> -n localink
kubectl logs <pod-name> -n localink
```
...
**Diagnosis**:
```bash
kubectl logs -f deployment/localink-backend -n localink | grep "MongoDB"
```
...
- Test connection from pod:
  ```bash
  kubectl exec -it <pod-name> -n localink -- sh
  curl -v <mongo-uri>
  ```
...
```bash
kubectl get services -n localink
kubectl describe service localink-frontend -n localink
```
...
kubectl port-forward service/localink-frontend 3000:80 -n localink

# Get events
kubectl get events -n localink --sort-by='.lastTimestamp'
```
